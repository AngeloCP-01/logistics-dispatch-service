...
describe("Assignment.markFailed", () => {
  it("fails from awaiting_driver and emits AssignmentFailed(all_offers_rejected)", () => {
    const a = a0();
    ...
    expect(events[0]).toBeInstanceOf(AssignmentFailed);
    expect((events[0] as AssignmentFailed).reason).toBe("all_offers_rejected");
  });
  it("refuses to fail when not awaiting_driver", () => {
    const a = a0();
    a.offerTo("att-1", DriverId.of(D1), NOW, EXP);
    expect(() => a.markFailed(EXP)).toThrow(InvariantViolationError);
  });
});
